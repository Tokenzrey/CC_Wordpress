...
import { Box, Lock, Mail, ArrowRight, Loader2, AlertCircle } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const [email, setEmail] = useState('demo@wpcube.local');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        ...
        setError('Invalid credentials. Try demo@wpcube.local');
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        <div className="flex justify-center gap-2 items-center cursor-pointer" onClick={() => navigate('/')}>
          ...
          <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">WPCube</h2>
        </div>
        ...
        <div className="bg-white py-8 px-4 shadow-xl shadow-slate-200/50 border border-slate-100 sm:rounded-xl sm:px-10">
          ...
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <div>
              <button
                ...
                className={`w-full flex justify-center py-2.5 px-4 border border-transparent rounded-lg shadow-sm text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all ${isLoggingIn ? 'opacity-70 cursor-not-allowed' : ''}`}
              >
                {isLoggingIn ? (
          ...
          <div className="mt-6 pt-6 border-t border-slate-100">
             <p className="text-xs text-center text-slate-400">
               Demo Credentials: <span className="font-mono text-slate-600">demo@wpcube.local</span> / <span className="font-mono text-slate-600">password</span>
             </p>
          </div>
        </div>
      </div>
