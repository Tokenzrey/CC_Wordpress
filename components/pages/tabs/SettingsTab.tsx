...
import { 
  Globe, Save, RefreshCw, AlertTriangle, Trash2, 
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Basic configuration for your instance.</p>
        </div>
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-2">Internal URL</label>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input 
                  ...
                  onChange={(e) => { setFormData({...formData, domain: e.target.value}); setDomainVerified(false); }}
              ...
              >
                {isVerifying ? <Loader2 className="w-4 h-4 animate-spin" /> : 
                 domainVerified ? <CheckCircle2 className="w-4 h-4" /> : <RefreshCw className="w-4 h-4" />}
                {isVerifying ? 'Verifying...' : domainVerified ? 'Verified' : 'Verify DNS'}
              </button>
            </div>
            {domainVerified && (
              <p className="text-xs text-green-600 mt-2 flex items-center gap-1">
                <CheckCircle2 className="w-3 h-3" /> CNAME record found. SSL will be provisioned automatically on save.
              </p>
            )}
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          ...
          <p className="text-xs text-slate-500 mt-0.5">Manage runtime environment and access.</p>
        </div>
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between border-b border-slate-100 pb-6">
            <div>
              <h4 className="text-sm font-bold text-slate-900">PHP Version</h4>
              <p className="text-xs text-slate-500">Change the PHP engine version. Requires restart.</p>
            </div>
            <select 
              value={formData.phpVersion}
              onChange={(e) => setFormData({...formData, phpVersion: e.target.value})}
              className="px-4 py-2 border border-slate-300 rounded-lg bg-white text-sm font-medium focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            >
              <option value="8.0">PHP 8.0 (Legacy)</option>
          ...
          <div className="flex items-center justify-between border-b border-slate-100 pb-6">
            <div>
              <h4 className="text-sm font-bold text-slate-900">WP_DEBUG Mode</h4>
              <p className="text-xs text-slate-500">Displays PHP errors on the frontend. Not recommended for production.</p>
            </div>
            <button 
              onClick={() => setFormData({...formData, debugMode: !formData.debugMode})}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${formData.debugMode ? 'bg-indigo-600' : 'bg-slate-200'}`}
            >
              <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${formData.debugMode ? 'translate-x-6' : 'translate-x-1'}`} />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500">Reset credentials for file transfer.</p>
            </div>
            <button 
      ...
      <div className="sticky bottom-6 bg-slate-900 text-white p-4 rounded-xl shadow-2xl flex justify-between items-center z-10">
        <span className="text-sm font-medium pl-2">Unsaved changes will be lost.</span>
        <button 
          onClick={handleSave}
          disabled={isSaving}
